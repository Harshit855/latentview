...
class RiskDeckSlide(slide.AbstractRiskDeckSlide):
    ...
    slide_title = 'Identity & Banking Decline Rate'
    ...
    def build_slide(self):
        ...
        data = df.groupby(['MONTH',"YEAR"])[['BAV_FRAUD','IDV_FRAUD','FUNDED_COUNT']].sum().reset_index()
        data['IDV_Decline'] = data['IDV_FRAUD']/data['FUNDED_COUNT']
        data['BAV_Decline'] = data['BAV_FRAUD']/data['FUNDED_COUNT']

        #updating chart 1
        chart1_df = pd.pivot_table(data = data,index='MONTH',columns='YEAR',values='IDV_Decline').reset_index()
        ...
        #updating chart 2
        chart2_df = pd.pivot_table(data = data,index='MONTH',columns='YEAR',values='BAV_Decline').reset_index()
