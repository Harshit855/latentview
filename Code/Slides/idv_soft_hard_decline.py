...
class RiskDeckSlide(slide.AbstractRiskDeckSlide):
    ...
    slide_title = 'Identity Soft & Hard Decline Rate'
    ...
    def build_slide(self):
        ...
        data = df.groupby(['MONTH',"YEAR"])[['IDV_FRAUD_SOFT','IDV_FRAUD_HARD','FUNDED_COUNT']].sum().reset_index()
        data['IDV_Soft_Decline'] = data['IDV_FRAUD_SOFT']/data['FUNDED_COUNT']
        data['IDV_Hard_Decline'] = data['IDV_FRAUD_HARD']/data['FUNDED_COUNT']

        #updating chart 1
        chart1_df = pd.pivot_table(data = data,index='MONTH',columns='YEAR',values='IDV_Soft_Decline').reset_index()
        ...
        #updating chart 2
        chart2_df = pd.pivot_table(data = data,index='MONTH',columns='YEAR',values='IDV_Hard_Decline').reset_index()
