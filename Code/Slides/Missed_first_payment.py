...
class RiskDeckSlide(slide.AbstractRiskDeckSlide):
    ...
    slide_title = 'Missed First Payment'
    ...
    def build_slide(self):
        ...
        data = df.groupby(['MONTH',"YEAR"])[['MISSED_FIRST_PAYMENT','FUNDED_COUNT']].sum().reset_index()
        data['MFP'] = data['MISSED_FIRST_PAYMENT']/data['FUNDED_COUNT']

        #updating chart 1
        chart1_df = pd.pivot_table(data = data,index='MONTH',columns='YEAR',values='MFP').reset_index()
